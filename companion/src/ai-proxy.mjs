...
export const createAiProxy = (apiKey = "", model = DEFAULT_MODEL, fetchImpl = globalThis.fetch) => {
  ...
  const generate = async (payload) => {
    ...
    if (payload === null || typeof payload !== "object" || !Array.isArray(payload.contents)) {
      return { ok: false, status: 400, error: "A Gemini generateContent body is required." };
    }
    const modelName = typeof payload.model === "string" && payload.model.length > 0
      ? payload.model
      : model;
    try {
      ...
      if (!response.ok) {
        return { ok: false, status: response.status, error: `Gemini returned ${response.status}.` };
      }
