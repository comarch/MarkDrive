// Self-contained export styles, loaded on demand so the fonts stay out of
// the application bundle. KaTeX CSS points at relative font files; each
// woff2 is inlined as a data URL so the exported HTML needs no network.
...
export function inlineKatexFonts(
  css: string,
  fontDataUris: Record<string, string>,
): string {
  return css.replace(
    /url\((['"]?)fonts\/(KaTeX_[A-Za-z0-9-]+\.woff2)\1\)/g,
    (match, _quote: string, fontFile: string) => {
      const dataUri = fontDataUris[fontFile];
      return dataUri ? `url("${dataUri}")` : match;
    },
  );
}

export const katexFontNames = Object.keys(fontDataUris);

export const katexInlinedCss = inlineKatexFonts(katexCss, fontDataUris);
export const highlightInlinedCss = hljsCss;
...
export const exportStyles: ExportStyles = {
  katexCss: katexInlinedCss,
  highlightCss: highlightInlinedCss,
};
